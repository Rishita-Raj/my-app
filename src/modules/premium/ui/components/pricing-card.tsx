...
export const PricingCard = ({
    variant,
    badge,
    price, features, title,description, priceSuffix, className, buttonText, onClick,
}: Props) => {
    return (
        <div className={cn(pricingCardVariants({variant}), className, "border")}>
            ...
            <div className="py-6">
                <Separator className="opacity-0 text-[#5D6B68]" />
            </div>
            <Button
            className="w-full"
            size="lg"
            ...
            <div className="flex flex-col gap-y-2 mt-6">
                ...
                {features.map((feature, index) => (
                    <li key={index} className="flex items-center gap-y-2.5">
                        <CircleCheckIcon
